...
s1series_path = 'B1_MAPPING_0010';

full_path = fullfile(full_dset_path, s1series_path);

[par1, img1] = dicomread2(full_path);

[Nx, Ny, Nz] = size(img1);

[~, I] = sort(par1.SliceLocation);

img1 = img1(:,:,I);

FlipAngle=par.FlipAngle;

s1series_path = 'B1_MAPPING_0011';

full_path = fullfile(full_dset_path, s1series_path);

[par2, img2] = dicomread2(full_path);

[~, I] = sort(par2.SliceLocation);

img2 = img2(:,:,I);

% b1map = acos(img1./max(img1, [], 'all'));
b1map = acos(img2./img1);
